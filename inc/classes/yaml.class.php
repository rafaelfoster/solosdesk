...
if (!function_exists('yaml_load')) {
  ...
  function yaml_load ($string) {
    return Yaml::YAMLLoadString($string);
  }
}
...
if (!function_exists('yaml_dump')) {
  ...
  function yaml_dump ($data) {
    return Yaml::YAMLDump($data, false, false, true);
  }
}
...
do {
  ...
  if (empty ($_SERVER['PHP_SELF']) || FALSE === strpos ($_SERVER['PHP_SELF'], 'Yaml.php') ) break;
  $file = $argv[1];
